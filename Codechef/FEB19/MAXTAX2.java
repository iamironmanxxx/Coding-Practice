...
class Abc
{
	...
	static long sumDFS(ArrayList<Integer> ar[],int s,long sum[],long nodeSum[])
	{
		if(compVisit[connected[s]-1]==0)
		{
			compVisit[connected[s]-1]=1;
			nodeSum[s]=sum[connected[s]-1];
		}
		...
		for(int i=0;i<ar[s].size();i++)
		{
			...
			{
				visited[v]=1;
				max=Math.max(max,sumDFS(ar,v,sum,nodeSum));
			}
		}
		if(max!=Integer.MIN_VALUE)
		{
			nodeSum[s]=(nodeSum[s]+max)%mod;
		}
		return nodeSum[s];
	}
