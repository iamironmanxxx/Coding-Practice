...
class Abc
{
	...
	public static void main(String args[])
	{
		...
		for(int g=0;g<t;g++)
		{
			...
			int vis[]=new int[color];
			for(int i=0;i<color;i++)
			{
				if(vis[i]==0)
				{
					DFSsum(scc,i,vis,nodeSum,sum);
				}
			}
	...
	public static void DFSsum(ArrayList<Integer> scc[],int ind,int vis[],long nodeSum[],long sum[])
	{
		vis[ind]=1;
		for(int i=0;i<scc[ind].size();i++)
		{
			...
			if(vis[v]==0)
			{
				DFSsum(scc,v,vis,nodeSum,sum);
			}
		}
		long max=0;
		for(int i=0;i<scc[ind].size();i++)
		{
			max=Math.max(max,nodeSum[scc[ind].get(i)]);
		}
		nodeSum[ind]=(sum[ind]+max)%mod;
	}
